<template>
<div id = "toiletusagestatus">
    <!-- Section 1 -->
    <section class="hero">
      <div class="container">
        <h1>Toilet Usage Status</h1>
        <p class="hero-desc">Time vs Toilet Usage Status — 1 means "ON", 0 means "OFF".</p>
      </div>
    </section>
    ...
    <section class="content">
        <div class="container">
            <div class="card chart-card">
                <h3>Toilet Usage Status vs time</h3>

                <div class = "chart-area">
                    <canvas ref="statusChartRef" aria-label="Toilet Usage Status vs time"></canvas>
                </div>
    ...
    return new Chart(ctx,{
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    display: true,
                    title: {display: true, text: "Toilet Usage Status"},
                    suggestedMin: -0.2,
                    suggestedMax: 1.2,
                    ticks: {
                        stepSize: 1,
...
async function loadData() {
    ...
    error.value = false;
    try {
        const resp = await axios.get("http://127.0.0.1:5000//api/toilet_usage");
        ...
        console.error("Failed to load toilet usage status: ", e);
        error.value = true;
    } finally {
        loading.value = false;
    }
