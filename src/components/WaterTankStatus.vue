<template>
<div id = "watertankstatus">
    <!-- Section 1 -->
    <section class="hero">
      <div class="container">
        <h1>Water Tank Status</h1>
        <p class="hero-desc">Time vs Water Tank Level — 1 means "ON", 0 means "OFF".</p>
      </div>
    </section>
    ...
    <section class="content">
        <div class="container">
            <div class="card chart-card">
                <h3>Water Tank Status vs time</h3>

                <div class = "chart-area">
                    <canvas ref="statusChartRef" aria-label="Water Tank Status vs time"></canvas>
                </div>
    ...
    return new Chart(ctx,{
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    display: true,
                    title: {display: true, text: "Water Tank Status"},
                    suggestedMin: -0.2,
                    suggestedMax: 1.2,
                    ticks: {
                        stepSize: 1,
...
async function loadData() {
    ...
    error.value = false;
    try {
        const resp = await axios.get("http://127.0.0.1:5000//api/water_tank_level");
        ...
        console.error("Failed to load water tank status: ", e);
        error.value = true;
    } finally {
        loading.value = false;
    }
